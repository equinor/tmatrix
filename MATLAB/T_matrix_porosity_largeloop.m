function out = T_matrix_porosity_largeloop(mineral_property, fluid_property, phi_vector,ALPHA,V,  frequency, angle,per_inc_con,per_inc_ani)
%% T-matrix approach
%% by
%% Remy Agersborg
%% Bergen, 20.02.2012
...
clear alpha
%[per_inc_ani,per_inc_con,phi_vector] =  equallengths(per_inc_ani,per_inc_con,phi_vector);

%% Looping over porosity to calculate vectors of elastic properties
[out.Vp, out.Vsv , out.Vsh , out.frequency , out.rho_eff , out.por] = deal(zeros(size(phi_vector)));
for ploop = 1:length(phi_vector)
    %% This part has been put into the loop, to model variations in connectivity and so on...
    alpha = ALPHA(ploop,:);
    v=V(ploop,:);
    tau = 10^-7 .*ones(size(alpha));
    ctrl = zeros(1,2);
    if (per_inc_ani(ploop) ~= 0 && per_inc_ani(ploop) ~= 1);
        ctrl(1) = 1;
    end
    if  per_inc_ani(ploop) == 1;
        ctrl(1) = 2;
    end
    ...
    if per_inc_ani(ploop) ~=0 && per_inc_ani(ploop) ~=1 ;
        ...
        alpha = cat(2,ALPHA(ploop,:),ALPHA(ploop,:));
        v     = cat(2,(1-per_inc_ani(ploop))*V(ploop,:), per_inc_ani(ploop)*V(ploop,:));
        tau   = cat(2,tau, tau);
    end
    
    %divide the porosity into isolated and connected part
    if per_inc_con(ploop) ~= 0  && per_inc_con(ploop) ~= 1
        ctrl(2) = 1;
    end
    if per_inc_con(ploop) == 1
        ctrl(2) =2;
    end
    ...
    initial_v.connected = v.*per_inc_con(ploop);
    initial_v.isolated = v.*(1-per_inc_con(ploop));
    %%
    ...
    %% This step performs a check, and if necessary reduces porosity to avoid unphysica
    ...
    [Vp, Vsv, Vsh ] = velocity_vti_angles( C_eff, rho_eff, angle_);
