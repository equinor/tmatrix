...
per_inc_con = 1 ;                   %percentage (0-1) of the inclusions which is connected 
...
%  stiff porosity example: max total porosity=0.45
 alpha(1) = 0.9;      v(1)= 0.99;    tau(1) = 10^-7;   % alpha: aspect ratio
 alpha(2) = 0.8;      v(2)= 0.01;    tau(2) = 10^-7;   % v : volume fraction
...
v(v.*porosity_range>alpha)=alpha(v.*porosity_range>alpha)/2;

% THE LINES BELOW ARE REMOVED, AND PROGRAM CHECK_ALPHA_CONCENTRATION IS NOT
% USED
% 
...
% LINES BELOW ARE PUT INTO THE MAIN PROGRAM, SO CTRL PARAMETER IS ESTIMATED
% THERE
%
% % ctrl = zeros(1,2);
...
%     out = T_matrix_porosity(mineral_property, fluid_property, in_alpha, in_v, tau,  frequency, angle_of_sym_plane,per_inc_con);
    out = T_matrix_porosity(mineral_property, fluid_property, porosity_range, alpha, v, tau,  frequency, angle_of_sym_plane,per_inc_con);
